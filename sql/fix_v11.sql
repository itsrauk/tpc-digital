...
-- e pagamentos variados. Parece que o sistema roda desde Jan/2026.
--
-- Lógica de desconto TPC:
--   R$250 = sempre o teto (valor cheio se pagar após dia 12)
--   Dia de semana noite  → desconto R$70  → paga R$180 até dia 12
--   Sábado               → desconto R$50  → paga R$200 até dia 12
--   Dia de semana tarde  → desconto R$100 → paga R$150 até dia 12
--   Segundo curso / Prod → desconto R$90  → paga R$160 até dia 12
--
--   No BD: amount = valor COM desconto, discount_amount = valor perdido após dia 12
--   amount + discount_amount = R$250 sempre.
--
-- Execute no Supabase SQL Editor.

-- ─── 0. Limpar dados sandbox anteriores ──────────────────────────
...
BEGIN
  ...
  -- Turma 1: Segunda-feira noite → desconto R$70
  -- Turma 2: Quarta-feira noite  → desconto R$70
  -- Turma 3: Sexta-feira (produção) → segundo curso → desconto R$90
...
-- ─── 4. Matrículas ────────────────────────────────────────────────
-- total_value = installments × 250 (teto sempre R$250/parcela integral)
...
BEGIN
  ...
  INSERT INTO enrollments (
    ...
    -- Teatro Basico — segunda noite → desconto R$70 → R$180 até dia 12
    ...
    -- Teatro Avancado — quarta noite → desconto R$70 → R$180 até dia 12
    ...
    -- Producao — sexta noite, segundo curso → desconto R$90 → R$160 até dia 12
    ('e2000006-0000-4000-8000-000000000006',
     ...
     1000, 90, 4, 'active', true)
  ON CONFLICT (id) DO NOTHING;
END;
$$;

-- ─── 5. Pagamentos ────────────────────────────────────────────────
-- amount        = o que o aluno REALMENTE pagou (ou vai pagar com desconto)
-- discount_amount = valor perdido se pagar após dia 12
-- amount + discount_amount = R$250 SEMPRE
--
-- Pagamento no prazo (até dia 12):
--   Turmas dia de semana → amount=180, discount_amount=70
--   Producao/2o curso   → amount=160, discount_amount=90
--
-- Pagamento com atraso (após dia 12, já marcado como pago):
--   amount=250, discount_amount=70 (ou 90) — perdeu o desconto, pagou o teto
--
-- Em atraso ainda não pago → amount=valor_com_desconto, discount_amount=70 ou 90
--   A UI mostra effective = amount+discount_amount = R$250

-- ── S1: Ana — boa pagadora (Jan-Mai pagos no prazo, Jun pendente) ──
INSERT INTO payments (student_id, enrollment_id, installment_number,
                      amount, discount_amount, due_date, status, paid_date) VALUES
  ('a2000001-0000-4000-8000-000000000001','e2000001-0000-4000-8000-000000000001',1, 180,70,'2026-01-12','paid','2026-01-08'),
  ('a2000001-0000-4000-8000-000000000001','e2000001-0000-4000-8000-000000000001',2, 180,70,'2026-02-12','paid','2026-02-10'),
  ('a2000001-0000-4000-8000-000000000001','e2000001-0000-4000-8000-000000000001',3, 180,70,'2026-03-12','paid','2026-03-07'),
  ('a2000001-0000-4000-8000-000000000001','e2000001-0000-4000-8000-000000000001',4, 180,70,'2026-04-12','paid','2026-04-09'),
  ('a2000001-0000-4000-8000-000000000001','e2000001-0000-4000-8000-000000000001',5, 180,70,'2026-05-12','paid','2026-05-08'),
  ('a2000001-0000-4000-8000-000000000001','e2000001-0000-4000-8000-000000000001',6, 180,70,'2026-06-12','pending',null);

-- ── S2: Bruno — inadimplente (Jan-Mar pagos, Abr-Mai em atraso) ───
INSERT INTO payments (student_id, enrollment_id, installment_number,
                      amount, discount_amount, due_date, status, paid_date) VALUES
  ('a2000002-0000-4000-8000-000000000002','e2000002-0000-4000-8000-000000000002',1, 180,70,'2026-01-12','paid','2026-01-11'),
  ('a2000002-0000-4000-8000-000000000002','e2000002-0000-4000-8000-000000000002',2, 180,70,'2026-02-12','paid','2026-02-12'),
  ('a2000002-0000-4000-8000-000000000002','e2000002-0000-4000-8000-000000000002',3, 180,70,'2026-03-12','paid','2026-03-10'),
  ('a2000002-0000-4000-8000-000000000002','e2000002-0000-4000-8000-000000000002',4, 180,70,'2026-04-12','overdue',null),
  ('a2000002-0000-4000-8000-000000000002','e2000002-0000-4000-8000-000000000002',5, 180,70,'2026-05-12','overdue',null),
  ('a2000002-0000-4000-8000-000000000002','e2000002-0000-4000-8000-000000000002',6, 180,70,'2026-06-12','pending',null);

-- ── S3: Carla — boa pagadora (Jan-Mai pagos, Jun pendente) ────────
INSERT INTO payments (student_id, enrollment_id, installment_number,
                      amount, discount_amount, due_date, status, paid_date) VALUES
  ('a2000003-0000-4000-8000-000000000003','e2000003-0000-4000-8000-000000000003',1, 180,70,'2026-01-12','paid','2026-01-05'),
  ('a2000003-0000-4000-8000-000000000003','e2000003-0000-4000-8000-000000000003',2, 180,70,'2026-02-12','paid','2026-02-05'),
  ('a2000003-0000-4000-8000-000000000003','e2000003-0000-4000-8000-000000000003',3, 180,70,'2026-03-12','paid','2026-03-05'),
  ('a2000003-0000-4000-8000-000000000003','e2000003-0000-4000-8000-000000000003',4, 180,70,'2026-04-12','paid','2026-04-05'),
  ('a2000003-0000-4000-8000-000000000003','e2000003-0000-4000-8000-000000000003',5, 180,70,'2026-05-12','paid','2026-05-09'),
  ('a2000003-0000-4000-8000-000000000003','e2000003-0000-4000-8000-000000000003',6, 180,70,'2026-06-12','pending',null);

-- ── S4: Diego — atrasou apenas maio ───────────────────────────────
INSERT INTO payments (student_id, enrollment_id, installment_number,
                      amount, discount_amount, due_date, status, paid_date) VALUES
  ('a2000004-0000-4000-8000-000000000004','e2000004-0000-4000-8000-000000000004',1, 180,70,'2026-01-12','paid','2026-01-09'),
  ('a2000004-0000-4000-8000-000000000004','e2000004-0000-4000-8000-000000000004',2, 180,70,'2026-02-12','paid','2026-02-08'),
  ('a2000004-0000-4000-8000-000000000004','e2000004-0000-4000-8000-000000000004',3, 180,70,'2026-03-12','paid','2026-03-11'),
  ('a2000004-0000-4000-8000-000000000004','e2000004-0000-4000-8000-000000000004',4, 180,70,'2026-04-12','paid','2026-04-10'),
  ('a2000004-0000-4000-8000-000000000004','e2000004-0000-4000-8000-000000000004',5, 180,70,'2026-05-12','overdue',null),
  ('a2000004-0000-4000-8000-000000000004','e2000004-0000-4000-8000-000000000004',6, 180,70,'2026-06-12','pending',null);

-- ── S5: Emilia — 3 meses em atraso; Fev pago tarde (sem desconto) ─
-- Jan pago no prazo → amount=180
-- Fev pago em 20/02 (após dia 12) → pagou R$250 (perdeu desconto)
--   amount=250 (valor que foi efetivamente pago), discount_amount=70
-- Mar/Abr/Mai: overdue → amount=180, discount_amount=70
--   UI exibe effective = 180+70 = R$250 (desconto perdido)
INSERT INTO payments (student_id, enrollment_id, installment_number,
                      amount, discount_amount, due_date, status, paid_date) VALUES
  ('a2000005-0000-4000-8000-000000000005','e2000005-0000-4000-8000-000000000005',1, 180,70,'2026-01-12','paid','2026-01-10'),
  ('a2000005-0000-4000-8000-000000000005','e2000005-0000-4000-8000-000000000005',2, 250,70,'2026-02-12','paid','2026-02-20'),
  ('a2000005-0000-4000-8000-000000000005','e2000005-0000-4000-8000-000000000005',3, 180,70,'2026-03-12','overdue',null),
  ('a2000005-0000-4000-8000-000000000005','e2000005-0000-4000-8000-000000000005',4, 180,70,'2026-04-12','overdue',null),
  ('a2000005-0000-4000-8000-000000000005','e2000005-0000-4000-8000-000000000005',5, 180,70,'2026-05-12','overdue',null),
  ('a2000005-0000-4000-8000-000000000005','e2000005-0000-4000-8000-000000000005',6, 180,70,'2026-06-12','pending',null);

-- ── S6: Felipe — Producao/2o curso (Mar-Abr pagos, Mai atraso) ────
-- Desconto R$90 → paga R$160 até dia 12 → R$250 se perder o desconto
INSERT INTO payments (student_id, enrollment_id, installment_number,
                      amount, discount_amount, due_date, status, paid_date) VALUES
  ('a2000006-0000-4000-8000-000000000006','e2000006-0000-4000-8000-000000000006',1, 160,90,'2026-03-12','paid','2026-03-08'),
  ('a2000006-0000-4000-8000-000000000006','e2000006-0000-4000-8000-000000000006',2, 160,90,'2026-04-12','paid','2026-04-07'),
  ('a2000006-0000-4000-8000-000000000006','e2000006-0000-4000-8000-000000000006',3, 160,90,'2026-05-12','overdue',null),
  ('a2000006-0000-4000-8000-000000000006','e2000006-0000-4000-8000-000000000006',4, 160,90,'2026-06-12','pending',null);

-- ─── 6. Frequência — últimas 3 semanas ───────────────────────────
-- Turma 1 (Segunda-feira): 04/Mai, 11/Mai, 18/Mai
-- Turma 2 (Quarta-feira):  06/Mai, 13/Mai, 20/Mai
...
-- ─── 7. Verificação ───────────────────────────────────────────────
SELECT 'alunos'      AS item, COUNT(*) FROM students    WHERE is_sandbox = true
UNION ALL
SELECT 'turmas',              COUNT(*) FROM classes     WHERE is_sandbox = true
UNION ALL
SELECT 'matriculas',          COUNT(*) FROM enrollments WHERE is_sandbox = true
UNION ALL
SELECT 'pagamentos',          COUNT(*) FROM payments p
...
SELECT
  ...
  -- Valor em aberto = amount + discount_amount para os em atraso (= R$250 cada)
  SUM(p.amount + p.discount_amount) FILTER (WHERE p.status = 'overdue') AS total_em_aberto
FROM students s
